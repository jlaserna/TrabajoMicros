	...
	switch (listaBloques.colision(personaje)) {
		case OBSTACULO:
			if (!jugadorInvencible) {
				...
				jugando = false;
				ETSIDI::playMusica("resources/music/trump.mp3", true);
			}
			break;
		case BONUSFANTASMA:
			bonusActivos[1] = true;
			...
			ETSIDI::play("resources/music/cash.mp3");
			break;
		case BONUSMONEDA:
			contadorMonedas++;
			ETSIDI::play("resources/music/moneda.mp3");
			break;
		case BONUSVELOCIDAD:
			bonusActivos[0] = true;
			...
			ETSIDI::play("resources/music/mordisco.mp3");
			break;
	}
...
void Mundo::Inicializa()
{
	...
	ETSIDI::playMusica("resources/music/himno.mp3", true);
	jugadorInvencible = false;
	jugando = false;
	contadorMonedas = 0;
